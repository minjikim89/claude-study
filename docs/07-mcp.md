...
이 챕터 개념을 바로 써보고 싶다면 → **[claude-code-catalog.vercel.app/ko/mcp](https://claude-code-catalog.vercel.app/ko/mcp)**

| 리소스 | 내용 |
|---|---|
| **MCP Servers 섹션** | 설치 가능한 MCP 서버 11개 (Notion, GitHub, Slack 등) + install 명령어 + 터미널 데모 |
| **블로그: Claude Code + Notion MCP** | Notion MCP 실전 설치 → 프로젝트 관리 자동화 전체 흐름 |
